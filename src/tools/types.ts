...
export interface Executable<TInput = unknown, TOutput = unknown>
   ...
   */
  validate(input: TInput): import('@/types').ValidationResult<TInput>;
  ...
  asFlowEffect(
    input?: TInput
  ): import('@/types').FlowEffect<
    TOutput,
    import('@/types').FlowError,
    import('@/types').FlowContext
  >;
}
...
export interface ExecutableMetadata {
  ...
  readonly resourceRequirements?: import('@/types').ResourceRequirements;
}
