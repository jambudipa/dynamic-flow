...
export interface VariableScope {
   ...
   */
  getParentScope(): VariableScope | undefined;
...
export interface WorkerPool {
   ...
   */
  executeParallel<T>(tasks: Array<() => Promise<T>>): Promise<T[]>;
...
export interface PauseResumeManager {
   ...
   */
  pause<T>(prompt: string): Promise<T>;
...
export class WorkerPoolImpl implements WorkerPool {
  ...
  async execute<T>(task: () => Promise<T>): Promise<T> {
    if (this.activeTasks >= this.maxWorkers) {
      await new Promise<void>((resolve) => this.taskQueue.push(resolve));
    }
  ...
  async executeParallel<T>(tasks: Array<() => Promise<T>>): Promise<T[]> {
...
export class PauseResumeManagerImpl implements PauseResumeManager {
  ...
  async pause<T>(prompt: string): Promise<T> {
    ...
    this.pausePromise = new Promise<T>((resolve) => {
      // Store as unknown-accepting callback; cast on resume
      this.resumeCallback = resolve as (value: unknown) => void;
    });

    return this.pausePromise as Promise<T>;
  }
