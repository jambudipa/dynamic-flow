...
import type { Tool, ToolJoin } from '../../lib';
import { Flow, Tools } from '../../lib';
...
export async function runExample(): Promise<{
  ...
  try {
      ...
      transform: Schema.transform(
        ...
        Schema.Struct({ text: Schema.String }), // To B's input
        {
          ...
          decode: (a: { title: string }) => {
            console.log(
              `Join A→B: Transforming { title: "${a.title}" } → { text: "${a.title}" }`
            );
            return { text: a.title };
          },
    ...
    const collected = await Effect.runPromise(
      Flow.runCollect(validProgram as Effect.Effect<any, any, never>, {
        name: 'Join A->B',
      })
    );
      ...
      transform: Schema.transform(
        ...
        Schema.Struct({ text: Schema.String, note: Schema.String }),
        {
          ...
          decode: (a: { title: string }) => {
            ...
            console.log(
              `Join A→C: Enriching { title: "${a.title}" } → { text: "${a.title}", note: "${note}" }`
            );
    ...
    const collectedC = await Effect.runPromise(
      Flow.runCollect(validCProgram as Effect.Effect<any, any, never>, {
        name: 'Join A->C',
      })
    );
    const enrichedResult = collectedC.output as any;
    console.log(
      'Length (with note, collected):',
      enrichedResult.length,
      '—',
      enrichedResult.note
    );
    console.log(
      'Execution time:',
      Duration.toMillis(collectedC.metadata.duration),
      'ms'
    );
        ...
        Effect.sync(() => {
          console.log(
            ...
            event.type === 'flow-complete'
              ? `→ length: ${(event as any).result?.length}`
              : ''
          );
        })
