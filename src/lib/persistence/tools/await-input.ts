...
export class AwaitInputTool<T> implements Tool<void, T> {
   ...
   */
  execute(input: void, context: ExecutionContext): Effect.Effect<T, ToolError, ToolRequirements> {
    const self = this;
    return Effect.gen(function* () {
      ...
      const suspensionContext: SuspensionContext = {
        toolId: self.id,
        awaitingInputSchema: self.awaitConfig.schema as Schema.Schema<unknown, unknown, never>,
      ...
      const suspensionSignal = new FlowSuspensionSignal({
        ...
        awaitingSchema: self.awaitConfig.schema as Schema.Schema<unknown, unknown, never>,
...
export class AwaitInputBuilder<T = unknown> {
  ...
  withMetadata(metadata: Record<string, unknown>): this {
    this.config = { ...this.config, metadata: { ...this.config.metadata, ...metadata } }
    return this
  }
  ...
  build(persistenceHub: PersistenceHub): AwaitInputTool<T> {
    ...
    return createAwaitInputTool(this.config as AwaitInputConfig<T>, persistenceHub)
  }
}
...
export const AwaitInputPresets = {
   ...
   * Wait for form data
   */
  form: <T>(id: string, name: string, description: string, schema: Schema.Schema<T>) =>
    awaitInput()
      .withId(id, name)
      .withDescription(description)
      .withSchema(schema)
...
export const createExponentialTimeoutAwaitInput = <T>(
  ...
  const timeoutMinutes = config.baseTimeoutMinutes * Math.pow(2, config.attemptNumber - 1)
