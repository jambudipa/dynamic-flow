...
import { AwaitInputTool, createAwaitInputTool, AwaitInputPresets } from './await-input'
...
export class HumanInTheLoopTools {
  ...
  approval(options: {
    ...
    return createAwaitInputTool({
      ...
      timeout: options.timeoutHours ? Duration.hours(options.timeoutHours) : undefined,
      metadata: {
        workflowType: 'approval',
        requireComments: options.requireComments || false,
        allowDelegation: options.allowDelegation || false
      }
    }, this.persistenceHub) as any
  }
  ...
  documentReview(options: {
    ...
    return createAwaitInputTool({
      ...
      timeout: options.timeoutDays ? Duration.days(options.timeoutDays) : undefined,
      metadata: {
  ...
  dataEntry<T>(options: {
    ...
    return createAwaitInputTool({
      ...
      timeout: options.timeoutMinutes ? Duration.minutes(options.timeoutMinutes) : undefined,
      metadata: {
  ...
  qualityCheck(options: {
    ...
    return createAwaitInputTool({
      ...
      schema: Schema.Struct({
        ...
        checklist: Schema.Array(Schema.Struct({
          item: Schema.String,
          status: Schema.Union(
            Schema.Literal('pass'),
            Schema.Literal('fail'),
...
export class CustomerInteractionTools {
  ...
  decision(options: {
    ...
    const choiceSchema = options.choices.length === 0
      ? Schema.String
      : options.choices.length === 1
      ? Schema.Literal(options.choices[0]!)
      : options.choices.length === 2
      ? Schema.Union(Schema.Literal(options.choices[0]!), Schema.Literal(options.choices[1]!))
      : Schema.Union(
          Schema.Literal(options.choices[0]!),
  ...
  manualTest(options: {
    ...
    return createAwaitInputTool({
      ...
      schema: Schema.Struct({
        ...
        stepResults: Schema.Array(Schema.Struct({
          step: Schema.String,
          result: Schema.Union(
            Schema.Literal('pass'),
            Schema.Literal('fail'),
  ...
  debugCheckpoint(options: {
    ...
    return createAwaitInputTool({
      ...
      schema: Schema.Struct({
        ...
        notes: Schema.String,
        stateModifications: Schema.optional(Schema.Record({
          key: Schema.String,
...
export class ToolsFactory {
   ...
   */
  textInput(id: string, name: string, description: string, timeoutMinutes?: number): AwaitInputTool<string> {
    return AwaitInputPresets.text(id, name, description)
      .withTimeout(timeoutMinutes ? Duration.minutes(timeoutMinutes) : Duration.hours(1))
   ...
   */
  confirmation(id: string, name: string, description: string, timeoutMinutes?: number): AwaitInputTool<boolean> {
  ...
  choice<T extends string>(
    ...
    const choiceSchema = choices.length === 0
      ? Schema.String
      : choices.length === 1
      ? Schema.Literal(choices[0]!)
      : choices.length === 2
      ? Schema.Union(Schema.Literal(choices[0]!), Schema.Literal(choices[1]!))
      : Schema.Union(
          Schema.Literal(choices[0]!),
    ...
    return this.custom({
      ...
      timeout: timeoutMinutes ? Duration.minutes(timeoutMinutes) : Duration.hours(1),
      metadata: {
 ...
 */
export const createToolsFactory = (persistenceHub: PersistenceHub): ToolsFactory => {
