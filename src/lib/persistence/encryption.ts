 ...
 */
export const AESStateEncryptor = Context.GenericTag<StateEncryptor>('@persistence/AESStateEncryptor')
...
export const AESStateEncryptorLive = (config: Partial<EncryptionConfig> = {}) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
        ...
        Effect.gen(function* () {
          ...
          const result = yield* Effect.try({
            try: () => {
              ...
              const cipher = crypto.createCipherGCM(finalConfig.algorithm, masterKey, iv)
      ...
      const decryptData = (
        ...
        Effect.gen(function* () {
          ...
          const decrypted = yield* Effect.try({
            try: () => {
              const crypto = require('crypto')
              const decipher = crypto.createDecipherGCM(finalConfig.algorithm, masterKey, iv)
       ...
       */
      const encrypt = (data: SerializedState): Effect.Effect<SerializedState, EncryptionError> =>
        Effect.gen(function* () {
          ...
          const iv = yield* Effect.sync(() => randomBytes(finalConfig.ivLength))
       ...
       */
      const decrypt = (data: SerializedState): Effect.Effect<SerializedState, EncryptionError> =>
        Effect.gen(function* () {
          ...
          if (encryptedPayload.algorithm !== finalConfig.algorithm) {
            return yield* Effect.fail(new EncryptionError({
              ...
              cause: { algorithm: encryptedPayload.algorithm, supported: finalConfig.algorithm }
            }))
          }
          ...
          if (!isKeyVersionCompatible(encryptedPayload.keyVersion)) {
            return yield* Effect.fail(new EncryptionError({
              ...
              cause: { keyVersion: encryptedPayload.keyVersion, currentVersion: keyVersion }
            }))
          }
          ...
          const { encrypted, encryptionAlgorithm, keyVersion: _keyVersion, ...cleanMetadata } = metadata
 ...
 */
export const NoOpEncryptor = Context.GenericTag<StateEncryptor>('@persistence/NoOpEncryptor')
 ...
 */
export const validateEncryptionKey = (key: string): Effect.Effect<boolean, EncryptionError> => {
  return Effect.try({
    try: () => {
...
export const createAESStateEncryptor = () => {
  ...
  if (!key) {
    throw new Error('DYNAMIC_FLOW_ENCRYPTION_KEY environment variable is required for encryption')
  }
