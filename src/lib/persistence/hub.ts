 ...
 */
export const PersistenceHubService = Context.GenericTag<IPersistenceHub>(
  '@persistence/PersistenceHub'
);

/**
 * Create persistence hub service layer
 */
export const PersistenceHubLive = (
  config: Partial<PersistenceHubConfig> = {}
) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      ): Effect.Effect<FlowStateCapture, PersistenceError> =>
        Effect.succeed({
      ...
      return {
        suspend: (flow: unknown, context: SuspensionContext) =>
          Effect.gen(function* () {
            ...
            const key =
              `susp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` as SuspensionKey;

            return {
              key,
              suspendedAt: new Date(),
              expiresAt: context.timeout
        ...
        cleanup: (criteria?: CleanupCriteria) =>
          Effect.succeed({ deletedCount: 0, errors: [] }),
 ...
 */
export const createDefaultPersistenceHub = (
  config?: Partial<PersistenceHubConfig>
) => PersistenceHubLive(config);
