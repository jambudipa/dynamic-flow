...
const DEFAULT_CONFIG: MongoConfig = {
  connectionString: process.env.MONGODB_URL || 'mongodb://localhost:27017/dynamicflow',
...
export class MongoStorageBackend implements StorageBackend {
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ Connected to MongoDB: ${this.config.databaseName}.${this.config.collectionName}`)
  ...
  private async ensureIndexes(): Promise<void> {
    try {
      ...
      console.warn(`Warning: Could not create MongoDB indexes: ${error instanceof Error ? error.message : 'Unknown error'}`)
    ...
    return Effect.gen(function* () {
      yield* Effect.tryPromise({
        try: async () => {
          ...
          await self.collection.replaceOne(
            { _id: key },
            document,
            { upsert: true }
          )
 ...
 */
export const createMongoBackend = (config?: Partial<MongoConfig>): StorageBackend => {
