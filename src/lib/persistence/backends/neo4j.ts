...
export class Neo4jStorageBackend implements StorageBackend {
  ...
  private async ensureDatabase(): Promise<void> {
    ...
    try {
      ...
      console.warn(`Warning: Could not auto-create Neo4j database: ${error instanceof Error ? error.message : 'Unknown error'}`)
    ...
    try {
      ...
      for (const query of SCHEMA_QUERIES) {
        try {
          ...
          console.warn(`Neo4j schema warning: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
      }
    ...
    return Effect.gen(function* () {
      yield* Effect.tryPromise({
        try: async () => {
          ...
          const session = self.driver.session({ database: self.config.database })
          ...
          try {
            ...
            if (self.config.enableRelationships && nodeProperties.flowId) {
              await self.createFlowRelationships(session, key, nodeProperties.flowId)
            }
   ...
   */
  private async createFlowRelationships(session: any, key: SuspensionKey, flowId: string): Promise<void> {
    try {
 ...
 */
export const createNeo4jBackend = (config?: Partial<Neo4jConfig>): StorageBackend => {
