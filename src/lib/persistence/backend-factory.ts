...
export class BackendFactory {
   ...
   */
  static create(options: BackendOptions): Effect.Effect<StorageBackend, StorageError> {
        ...
        case 'filesystem': {
          ...
          case 'postgres': {
            ...
            if (options.connectionString) {
              ...
              return createPostgresBackendFromUrl(options.connectionString, options.config as any)
            } else {
          ...
          case 'redis': {
            ...
            if (options.connectionString) {
              ...
              return createRedisBackendFromUrl(options.connectionString, options.config as any)
            } else {
          ...
          case 'mongodb': {
            ...
            if (options.connectionString) {
              ...
              return createMongoBackendFromUrl(options.connectionString, options.config as any)
            } else {
   ...
   */
  static fromConfig(config: PersistenceConfig): Effect.Effect<StorageBackend, StorageError> {
    return this.create({
      type: config.backend,
  ...
  static fromEnvironment(): Effect.Effect<StorageBackend, StorageError> {
    return Effect.gen(function* () {
      ...
      const backendType = (process.env.DYNAMICFLOW_PERSISTENCE_BACKEND || 'filesystem') as BackendType
      // Get connection string from environment
      const connectionString = BackendFactory.getConnectionStringFromEnv(backendType)
   ...
   */
  public static getConnectionStringFromEnv(backendType: BackendType): string | undefined {
    switch (backendType) {
      case 'postgres':
   ...
   */
  private static getConfigFromEnv(backendType: BackendType): Record<string, unknown> {
    ...
    switch (backendType) {
      ...
      case 'postgres':
        ...
        if (process.env.DYNAMICFLOW_PG_MAX_CONNECTIONS) {
          config.maxConnections = parseInt(process.env.DYNAMICFLOW_PG_MAX_CONNECTIONS)
        }
        break
      case 'redis':
   ...
   */
  static isBackendAvailable(backendType: BackendType): Effect.Effect<boolean, never> {
    switch (backendType) {
      case 'filesystem':
   ...
   */
  static getBackendStatus(): Effect.Effect<Array<{ type: BackendType; available: boolean }>, never> {
    return Effect.gen(function* () {
 ...
 */
export const createBackendFromEnvironment = (): Effect.Effect<StorageBackend, StorageError> => {
  return BackendFactory.fromEnvironment()
}
/**
 * Convenience function to create backend from config
 */
export const createBackendFromConfig = (config: PersistenceConfig): Effect.Effect<StorageBackend, StorageError> => {
  return BackendFactory.fromConfig(config)
}
/**
 * Auto-detect and create the best available backend
 */
export const createBestAvailableBackend = (): Effect.Effect<StorageBackend, StorageError> => {
  return Effect.gen(function* () {
    
    // Priority order for backend selection
    const backendPriority: BackendType[] = ['postgres', 'mongodb', 'redis', 'neo4j', 'filesystem']
    ...
    for (const backend of backendPriority) {
      if (availableBackends.includes(backend)) {
        ...
        return yield* BackendFactory.create({
          type: backend,
          connectionString: BackendFactory.getConnectionStringFromEnv(backend) || undefined
        })
      }
    }
