 ...
 */
export const EffectStateSerializer = Context.GenericTag<StateSerializer>('@persistence/EffectStateSerializer')
...
export const EffectStateSerializerLive = Layer.effect(
  ...
  Effect.gen(function* () {
     ...
     */
    const serialize = (state: unknown): Effect.Effect<SerializedState, SerializationError> =>
      Effect.gen(function* () {
    ...
    return {
      serialize,
      deserialize: (data: SerializedState) => Effect.gen(function* () {
        ...
        if (calculatedChecksum !== data.metadata.checksum) {
          return yield* Effect.fail(new SerializationError({
            ...
            cause: { expected: data.metadata.checksum, calculated: calculatedChecksum }
          }))
        }
      ...
      compress: (data: SerializedState) => Effect.gen(function* () {
      ...
      decompress: (data: CompressedState) => Effect.gen(function* () {
 ...
 */
export const estimateCompressionRatio = (state: unknown): Effect.Effect<number, SerializationError | CompressionError, StateSerializer> => {
 ...
 */
export const validateStateSerializability = (state: unknown): Effect.Effect<boolean, SerializationError, StateSerializer> => {
  return Effect.gen(function* () {
    const serializer = yield* EffectStateSerializer
    const serialized = yield* serializer.serialize(state)
