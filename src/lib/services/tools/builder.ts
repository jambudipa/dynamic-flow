...
import { ToolError as ErrorsToolError, ValidationError, LLMError } from '../../errors'
import type { Tool, ToolMetadata, ExecutionContext, ToolRequirements } from '../../tools/types'
...
export interface ToolConfig<TInput = any, TOutput = any> {
  ...
  execute: (input: TInput) => Effect.Effect<TOutput, any> | Promise<TOutput> | TOutput
  validate?: (input: TInput) => Effect.Effect<boolean, ValidationError> | boolean
 ...
 */
export class ToolBuilderService extends Effect.Service<ToolBuilderService>()('ToolBuilderService', {
  effect: Effect.gen(function* () {
    ...
    return {
      ...
      createTool: <TInput, TOutput>(config: ToolConfig<TInput, TOutput>) =>
        Effect.gen(function* () {
          ...
          if (!config.id || !config.name) {
            return yield* Effect.fail(new ErrorsToolError({
          ...
          const tool: Tool<TInput, TOutput> = {
            ...
            execute: (input: TInput, context: ExecutionContext): Effect.Effect<TOutput, TypesToolError, ToolRequirements> => {
              ...
              if (Effect.isEffect(result)) {
                ...
                return Effect.tryPromise({
                  ...
                  catch: (error) => new TypesToolError({
                    toolId: config.id,
                    phase: 'execution',
                    cause: error
                  })
            ...
            inputSchema: config.inputSchema || Schema.Unknown as Schema.Schema<TInput>,
            outputSchema: config.outputSchema || Schema.Unknown as Schema.Schema<TOutput>
          }
      ...
      createLLMTool: (config: LLMToolConfig) =>
        Effect.gen(function* () {
          ...
          const tool: Tool<unknown, unknown> = {
            ...
            execute: (input: any, context: ExecutionContext): Effect.Effect<any, TypesToolError, ToolRequirements> =>
              Effect.gen(function* () {
                ...
                // Generate completion
                const response = yield* llmService.generateCompletion(prompt, {
                  systemPrompt: config.systemPrompt,
                  model: config.model,
                  temperature: config.temperature,
                  maxTokens: config.maxTokens
                }).pipe(
                  Effect.mapError(error => new TypesToolError({
                    toolId: config.id,
                    phase: 'execution',
                    cause: error
                  }))
                ...
                if (config.outputSchema) {
                  const parsed = yield* Effect.try(() => JSON.parse(response.text)).pipe(
                    Effect.mapError(error => new TypesToolError({
                      toolId: config.id,
                      phase: 'cleanup',
                      cause: error,
                      details: { reason: 'Failed to parse LLM response as JSON' }
                    }))
                  )
                  
                  return yield* Schema.decodeUnknown(config.outputSchema)(parsed).pipe(
                    Effect.mapError(error => new TypesToolError({
                      toolId: config.id,
                      phase: 'cleanup',
                      cause: error,
                      details: { reason: 'LLM response does not match output schema' }
                    }))
                  )
                }
      ...
      fromFunction: <T, R>(
        fn: (input: T) => R,
        metadata: ToolMetadata & { id: string; name: string; description?: string }
      ) =>
        Effect.gen(function* () {
          const tool: Tool<unknown, unknown> = {
            id: metadata.id,
            name: metadata.name,
            description: metadata.description || `Function tool: ${metadata.name}`,
            version: '1.0.0',
            
            execute: (input: unknown, context: ExecutionContext): Effect.Effect<unknown, TypesToolError, ToolRequirements> => 
       ...
       */
      compose: (tools: Tool[], metadata?: { id: string; name: string; description?: string }) =>
        Effect.gen(function* () {
          if (tools.length === 0) {
            return yield* Effect.fail(new ErrorsToolError({
          ...
          const composedTool: Tool<unknown, unknown> = {
            ...
            description: metadata?.description || `Composition of ${tools.length} tools`,
            version: '1.0.0',
            ...
            execute: (input: any, context: ExecutionContext) =>
              Effect.reduce(
                tools,
                input,
                (currentInput, tool) => tool.execute(currentInput, context)
              ),
