 ...
 */
export class ToolRegistryService extends Effect.Service<ToolRegistryService>()(
  'ToolRegistryService',
  {
    effect: Effect.gen(function* () {
      // Internal registry state
      const tools = yield* Ref.make<HashMap.HashMap<string, Tool>>(
        HashMap.empty()
      );
 ...
 * Test implementation
 */
export const ToolRegistryTest = Layer.succeed(ToolRegistryService, {
  register: () => Effect.void,
  unregister: () => Effect.void,
  get: (id: string) =>
    Effect.fail(new ToolNotFoundError({ toolId: id, operation: 'get' })),
  list: () => Effect.succeed([]),
  clear: () => Effect.void,
  ...
  getLLM: (id: string) =>
    Effect.fail(new ToolNotFoundError({ toolId: id, operation: 'getLLM' })),
