...
import { Tool, ToolRegistry, LLMTool } from '../../tools/types'
import { RegistrationError as TypesRegistrationError, ToolNotFoundError as TypesToolNotFoundError, ValidationError as TypesValidationError } from '../../tools/types'
...
export class ToolRegistryImpl implements ToolRegistry {
  ...
  register<TInput, TOutput>(tool: Tool<TInput, TOutput>): Effect.Effect<void, TypesRegistrationError, never> {
  ...
  get(id: string): Effect.Effect<Tool<unknown, unknown>, TypesToolNotFoundError, never> {
    return Effect.gen(function* () {
      const registry = yield* ToolRegistryService
  ...
  registerLLM<TInput, TOutput>(tool: LLMTool<TInput, TOutput>): Effect.Effect<void, TypesRegistrationError, never> {
    // Convert LLMTool to Tool and register
    return this.register(tool as unknown as Tool<TInput, TOutput>)
  }
  
  getLLM(id: string): Effect.Effect<LLMTool<unknown, unknown>, TypesToolNotFoundError, never> {
  ...
  listByCategory(category: string): Effect.Effect<Tool<unknown, unknown>[], never, never> {
  ...
  unregister(id: string): Effect.Effect<void, TypesToolNotFoundError, never> {
    return Effect.gen(function* () {
      ...
      if (!exists) {
        return yield* Effect.fail(new TypesToolNotFoundError({
          toolId: id,
          context: 'unregister'
        }))
      }
  ...
  validateInput(toolId: string, input: unknown): Effect.Effect<void, TypesValidationError, never> {
    const self = this
    return Effect.gen(function* () {
      const tool = yield* self.get(toolId).pipe(
        Effect.mapError(() => new TypesValidationError({
  ...
  validateOutput(toolId: string, output: unknown): Effect.Effect<void, TypesValidationError, never> {
    const self = this
    return Effect.gen(function* () {
      const tool = yield* self.get(toolId).pipe(
        Effect.mapError(() => new TypesValidationError({
