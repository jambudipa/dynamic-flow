 ...
 */
export class FlowGeneratorService extends Effect.Service<FlowGeneratorService>()(
    ...
    effect: Effect.gen(function* () {
      ...
      const generateFromPrompt = (
        ...
        Effect.gen(function* (): any {
          ...
          const generatedFlow = yield* llmService
            .generateStructured(prompt, FlowSchema, {
              systemPrompt,
              model: options?.model,
          ...
          if (options?.validateOutput !== false) {
            const validationResult = yield* validationService
              .validateFlow(generatedFlow as any)
              .pipe(
                Effect.mapError(
                  (error) =>
                    new GenerationError({
      ...
      return {
        ...
        optimizeFlow: (flow: Flow, criteria?: string[]) =>
          ...
          4. Maintains the same functionality`;
            ...
            const optimizedResult = yield* generateFromPrompt(
              optimizationPrompt,
              {
                validateOutput: true,
              }
        ...
        generateDocumentation: (flow: Flow) =>
          ...
          5. Configuration options`;
            ...
            const documentation = yield* llmService.generateCompletion(
              docPrompt,
              {
                maxTokens: 1500,
              }
        ...
        suggestImprovements: (flow: Flow) =>
          ...
          - Best practices`;
            ...
            const suggestions = yield* llmService.generateCompletion(
              analysisPrompt,
              {
                temperature: 0.8,
                maxTokens: 1000,
        ...
        convertFlow: (flow: Flow, targetFormat: 'yaml' | 'xml' | 'mermaid') =>
          ...
          ${JSON.stringify(flow, null, 2)}`;
            ...
            const converted = yield* llmService.generateCompletion(
              conversionPrompt,
              {
                temperature: 0.3,
                maxTokens: 2000,
      ...
      function getTemplate(
        name: string
      ): Effect.Effect<FlowTemplate | null, never> {
            ...
            generate: (params) => ({
              ...
              nodes: [
                { id: 'start', type: 'start' },
                {
                  id: 'extract',
                  type: 'tool',
                  config: { tool: params.source },
                },
                {
                  id: 'transform',
                  type: 'map',
                  config: { with: params.transformations },
                },
                {
                  id: 'load',
                  type: 'tool',
                  config: { tool: params.destination },
                },
                { id: 'end', type: 'end' },
              ],
              edges: [
