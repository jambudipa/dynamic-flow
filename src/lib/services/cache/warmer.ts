import { Effect, Schedule, Duration, Fiber, Ref, HashMap, Option, Context, Layer } from 'effect'
...
export interface CacheWarmerService {
  readonly registerWarmingTask: (config: CacheWarmingConfig) => Effect.Effect<string, CacheError, never>
  readonly warmKeys: (keys: string[], dataLoader: (key: string) => Effect.Effect<any, Error>) => Effect.Effect<{ warmed: number; failed: number; results: Array<{ key: string; success: boolean; error?: Error }> }, never>
  readonly startEagerWarming: (taskId: string) => Effect.Effect<void, CacheError, never>
  readonly startScheduledWarming: (taskId: string) => Effect.Effect<void, CacheError, never>
  readonly startPredictiveWarming: (taskId: string) => Effect.Effect<void, CacheError, never>
  readonly stopWarming: (taskId: string) => Effect.Effect<void, CacheError, never>
  readonly getStats: () => Effect.Effect<WarmingStats, never>
  readonly predictAccess: (pattern: string) => Effect.Effect<string[], never>
}

export const CacheWarmerService = Context.GenericTag<CacheWarmerService>('CacheWarmerService')
...
export const CacheWarmerServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const warmingTasks = yield* (Ref.make<HashMap.HashMap<string, WarmingTask>>(HashMap.empty()))
    
    // Access patterns for predictive warming
    const accessPatterns = yield* (Ref.make<Map<string, { count: number; lastAccess: Date }>>(new Map()))
    
    // Helper function to warm keys
    const warmKeysImpl = (keys: string[], dataLoader: (key: string) => Effect.Effect<any, Error>) =>
      Effect.gen(function* () {
    ...
    const startScheduledWarmingImpl = (task: WarmingTask) =>
      Effect.gen(function* () {
        if (!task.config.schedule) {
          return yield* (Effect.fail(new CacheError({
    ...
    const startPredictiveWarmingImpl = (task: WarmingTask) =>
      Effect.gen(function* () {
        ...
        if (predictedKeys.length > 0) {
          const result = yield* (warmKeysImpl(predictedKeys, task.config.dataLoader))
    ...
    return {
      registerWarmingTask: (config: CacheWarmingConfig): Effect.Effect<string, CacheError, never> =>
        Effect.gen(function* () {
          ...
          yield* (Ref.update(warmingTasks, map => HashMap.set(map, taskId, task)))
      ...
      warmKeys: (keys: string[], dataLoader: (key: string) => Effect.Effect<any, Error>) => warmKeysImpl(keys, dataLoader),
      
      startEagerWarming: (taskId: string): Effect.Effect<void, CacheError, never> =>
        Effect.gen(function* () {
      ...
      startScheduledWarming: (taskId: string): Effect.Effect<void, CacheError, never> =>
        Effect.gen(function* () {
          const tasks = yield* (Ref.get(warmingTasks))
          const task = HashMap.get(tasks, taskId)
      ...
      startPredictiveWarming: (taskId: string): Effect.Effect<void, CacheError, never> =>
        Effect.gen(function* () {
          const tasks = yield* (Ref.get(warmingTasks))
          const task = HashMap.get(tasks, taskId)
      ...
      getStats: (): Effect.Effect<WarmingStats, never, never> =>
        Effect.gen(function* () {
          ...
          return {
            ...
            totalItemsWarmed: taskList.reduce((sum, t) => sum + (t.itemsWarmed || 0), 0),
