...
export const InMemoryCacheLive = (options?: {
  ...
  const defaultTtl = options?.defaultTtl ? Duration.toMillis(options.defaultTtl) : 3600000 // 1 hour
  ...
  return Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const evictLRU = Effect.gen(function* () {
        ...
        if (lruKey !== null) {
          yield* (Ref.update(stateRef, s => ({
            ...
            stats: { ...s.stats, evictions: s.stats.evictions + 1, size: s.stats.size - 1 }
          })))
        }
      })
      ...
      return {
        get: <T>(key: string) => Effect.gen(function* () {
          const state = yield* (Ref.get(stateRef))
          ...
          if (isExpired(cacheEntry)) {
            ...
            yield* (Ref.update(stateRef, s => ({
              ...
              stats: { ...s.stats, misses: s.stats.misses + 1, size: s.stats.size - 1 }
            })))
            return Option.none<T>()
          }
          ...
          yield* (Ref.update(stateRef, s => ({
            entries: HashMap.set(s.entries, key, updatedEntry as CacheEntry<unknown>),
        ...
        set: <T>(key: string, value: T, ttl?: number) => Effect.gen(function* () {
          const state = yield* (Ref.get(stateRef))
          
          // Check if we need to evict
          if (state.stats.size >= maxSize && !HashMap.has(state.entries, key)) {
            yield* (evictLRU)
          }
          ...
          yield* (Ref.update(stateRef, s => ({
            entries: HashMap.set(s.entries, key, entry as CacheEntry<unknown>),
            stats: { 
              ...s.stats, 
        ...
        delete: (key: string) => Effect.gen(function* () {
          yield* (Ref.update(stateRef, s => ({
            ...
            stats: { 
              ...s.stats, 
              size: HashMap.has(s.entries, key) ? s.stats.size - 1 : s.stats.size 
            }
          })))
        }),
        ...
        invalidate: (pattern: string) => Effect.gen(function* () {
