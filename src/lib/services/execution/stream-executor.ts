import { Effect, Stream, Chunk, Queue, Fiber, Ref, Schedule, Duration } from 'effect'
 ...
 */
export class StreamExecutorService extends Effect.Service<StreamExecutorService>()('StreamExecutorService', {
  effect: Effect.gen(function* () {
    ...
    const service = {
       ...
       */
      executeStream: (graph: IRGraph, input: any, options?: StreamExecutionOptions) =>
        Effect.gen(function* () {
          ...
          const fiber = yield* Effect.fork(
            executeGraphStreaming(graph, input, eventQueue, cancelled, options)
          )
       ...
       */
      executeNodeStream: (node: IRNode, input: Stream.Stream<any, never>, options?: StreamExecutionOptions) =>
        Effect.succeed(() => {
          ...
          const processedStream = input.pipe(
            Stream.mapEffect((item) =>
              executeNode(node, item).pipe(
                Effect.retry({
                  schedule: errorStrategy === 'retry' 
                    ? Schedule.recurs(maxRetries).pipe(
                        Schedule.addDelay(() => Duration.millis(options?.retryDelay || 1000))
                      )
                    : Schedule.stop
                }),
      ...
      createPipeline: (nodes: IRNode[], options?: StreamExecutionOptions) =>
        Effect.gen(function* () {
          return (input: Stream.Stream<any, never>) => {
            return nodes.reduce<Stream.Stream<any, never, never>>((stream, node) => {
              return stream.pipe(
                Stream.flatMap((item) =>
                  Stream.fromEffect(
                    service.executeNodeStream(node, Stream.succeed(item), options).pipe(
                      Effect.map(fn => fn())
                    )
                  )
                )
       ...
       */
      mergeStreams: (streams: Stream.Stream<any, never>[], strategy: 'concat' | 'merge' | 'zip' = 'merge') =>
        Effect.gen(function* () {
          switch (strategy) {
            case 'concat':
       ...
       */
      applyBackpressure: (stream: Stream.Stream<any, never>, strategy: 'drop' | 'buffer' | 'block', limit: number) =>
        Effect.gen(function* () {
          switch (strategy) {
            case 'drop':
       ...
       */
      windowStream: (stream: Stream.Stream<any, never>, windowSize: number, windowType: 'count' | 'time') =>
        Effect.gen(function* () {
          if (windowType === 'count') {
            return stream.pipe(
              Stream.grouped(windowSize)
            )
          } else {
      ...
      monitorProgress: (stream: Stream.Stream<any, never>, total?: number) =>
        Effect.gen(function* () {
          ...
          return stream.pipe(
            Stream.tap(() =>
              Effect.gen(function* () {
                const count = yield* Ref.updateAndGet(processed, n => n + 1)
       ...
       */
      batchProcess: (stream: Stream.Stream<any, never>, batchSize: number, processor: (batch: any[]) => Effect.Effect<any, Error>) =>
        stream.pipe(
          Stream.grouped(batchSize),
          Stream.mapEffect((batch) =>
            processor(Chunk.toArray(batch))
          )
        ),
       ...
       */
      rateLimit: (stream: Stream.Stream<any, never>, itemsPerSecond: number) =>
        stream.pipe(
          Stream.schedule(
       ...
       */
      deduplicate: (stream: Stream.Stream<any, never>, keyFn: (item: any) => string) =>
        Effect.gen(function* () {
    ...
    // Helper to execute node
    function executeNode(node: IRNode, input: any): Effect.Effect<any, ExecutionError> {
