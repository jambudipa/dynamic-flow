 ...
 */
export class PauseResumeService extends Effect.Service<PauseResumeService>()('PauseResumeService', {
  effect: Effect.gen(function* () {
    const persistence = yield* PersistenceService
    
    // In-memory pause states
    const pausedFlows = yield* Ref.make<HashMap.HashMap<string, PauseState>>(HashMap.empty())
    const checkpoints = yield* Ref.make<HashMap.HashMap<string, ExecutionCheckpoint[]>>(HashMap.empty())
    
    return {
      /**
       * Pause flow execution
       */
      pause: (flowId: string, nodeId: string, state: Record<string, any>, reason?: string) =>
        Effect.gen(function* () {
          // Create checkpoint
      ...
      resume: (flowId: string, options?: ResumeOptions) =>
        Effect.gen(function* () {
          ...
          if (Option.isNone(pauseState)) {
            ...
            const persistedState = yield* persistence.load(`pause-${flowId}`).pipe(
              Effect.mapError(() => new StateError({
            ...
            return {
              ...
              state: options?.overrideState || (persistedState as any).checkpoint.state,
              resumeFrom: options?.skipToNode || (persistedState as any).checkpoint.nodeId
            }
          }
       ...
       * Create a checkpoint
       */
      checkpoint: (flowId: string, nodeId: string, state: Record<string, any>) =>
        Effect.gen(function* () {
          const checkpoint: ExecutionCheckpoint = {
      ...
      listCheckpoints: (flowId: string) =>
        Effect.gen(function* () {
          ...
          if (Option.isNone(flowCheckpoints)) {
            // Try to load from persistence
            const persisted = yield* persistence.load(`checkpoints-${flowId}`).pipe(
      ...
      getCheckpoint: (checkpointId: string) =>
        Effect.gen(function* () {
          ...
          for (const [_, flowCheckpoints] of allCheckpoints) {
            const checkpoint = flowCheckpoints.find(cp => cp.id === checkpointId)
            if (checkpoint) {
              return checkpoint
            }
          }
          ...
          // Try to load from persistence
          const persisted = yield* persistence.load(`checkpoint-${checkpointId}`).pipe(
            Effect.mapError(() => new StateError({
      ...
      getPauseState: (flowId: string) =>
        Effect.gen(function* () {
          ...
          if (Option.isNone(state)) {
            // Try persistence
            return yield* persistence.load(`pause-${flowId}`).pipe(
    ...
    function saveCheckpoint(checkpoint: ExecutionCheckpoint) {
      return Effect.gen(function* () {
        ...
        const allCheckpoints = yield* Ref.get(checkpoints)
        const flowCheckpoints = Option.getOrElse(HashMap.get(allCheckpoints, checkpoint.flowId), () => [] as ExecutionCheckpoint[])
        yield* persistence.save(`checkpoints-${checkpoint.flowId}`, flowCheckpoints)
      })
    }
  })
}) {}