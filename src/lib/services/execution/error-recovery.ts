 ...
 */
export class ErrorRecoveryService extends Effect.Service<ErrorRecoveryService>()('ErrorRecoveryService', {
  effect: Effect.gen(function* () {
    // Circuit breaker states
    const circuitBreakers = yield* Ref.make<HashMap.HashMap<string, CircuitBreakerState>>(HashMap.empty())
    ...
    return {
      ...
      executeWithRecovery: <T>(
        ...
        Effect.gen(function* () {
          ...
          switch (config.strategy) {
            ...
            default:
              return yield* Effect.fail(new RecoveryError({
      ...
      createRetrySchedule: (config: RecoveryConfig) => {
        ...
        return Schedule.exponential(Duration.millis(baseDelay), multiplier).pipe(
       ...
       * Apply compensation
       */
      applyCompensation: (compensations: Array<() => Effect.Effect<void, Error>>) =>
        Effect.gen(function* () {
       ...
       */
      createFallbackChain: <T>(operations: Array<() => Effect.Effect<T, Error>>) =>
        Effect.gen(function* () {
          let lastError: Error | undefined
          ...
          return yield* Effect.fail(lastError || new Error('All fallbacks failed'))
        }),
      
      /**
       * Bulkhead pattern - limit concurrent executions
       */
      bulkhead: <T>(operation: () => Effect.Effect<T, Error>, maxConcurrent: number) => {
      ...
      timeoutWithFallback: <T>(
        ...
        operation().pipe(
          ...
          Effect.map((opt) => Option.match(opt as Option.Option<T>, {
            onNone: () => fallback,
            onSome: (value) => value
          }))
        ),
          ...
          Effect.catchAll((error) =>
            Effect.gen(function* () {
              if (config.compensationAction) {
                yield* config.compensationAction().pipe(
