import { Effect, Ref, Option, Duration, FiberRef, HashMap, Layer } from 'effect'
import { v4 as uuidv4 } from 'uuid'
import { ExecutionError } from '../../errors'
import type { VariableScope, WorkerPool, PauseResumeManager, FlowControlManager } from '../../core/context/execution-context'
 ...
 */
export class ExecutionContextService extends Effect.Service<ExecutionContextService>()('ExecutionContextService', {
  effect: Effect.gen(function* () {
    // Core state
    const variables = yield* Ref.make<HashMap.HashMap<string, unknown>>(HashMap.empty())
    const metadata = yield* Ref.make<HashMap.HashMap<string, unknown>>(HashMap.empty())
    ...
    const parentScopes = yield* Ref.make<Array<HashMap.HashMap<string, unknown>>>([])
    ...
    // Pause/resume state
    const pauseState = yield* Ref.make<Option.Option<{
      prompt: string
    ...
    return {
        ...
        Effect.gen(function* () {
          ...
          if (active >= max) {
            yield* Effect.async<void>((resume) => {
              Ref.update(taskQueue, queue => [...queue, () => resume(Effect.void)])
                .pipe(Effect.runSync)
            })
          }
          ...
          try {
            const result = yield* Effect.tryPromise({
              try: task,
              catch: (error) => new ExecutionError({
      ...
      pause: <T>(prompt: string) =>
        Effect.async<T, never>((resume) => {
          Ref.update(pauseState, () => 
            Option.some({
              prompt,
              resolver: (value: unknown) => resume(Effect.succeed(value as T))
            })
          ).pipe(Effect.runSync)
        }),
      ...
      createChildContext: () =>
        Effect.gen(function* () {
          ...
          yield* Ref.update(parentScopes, parents => [...parents, currentVars])
          yield* Ref.set(variables, HashMap.empty())
 ...
 * Test implementation
 */
export const ExecutionContextTest = Layer.succeed(
  ExecutionContextService,
  {
    getVariable: () => Effect.fail(new ExecutionError({ message: 'Variable not found', phase: 'execution' })),
    ...
    submitTask: () => Effect.fail(new ExecutionError({ message: 'Task failed', phase: 'execution' })),
