 ...
 */
export const ReduceOperatorService = Context.GenericTag<ReduceOperatorService>('ReduceOperatorService')
...
export const DefaultReduceOperatorService = Effect.gen(function* () {
  ...
  return ReduceOperatorService.of({
      ...
      Effect.gen(function* () {
        ...
        for (let i = 0; i < items.length; i++) {
          try {
            ...
            yield* context.setVariable(`${config.reduce}_accumulator`, accumulator)
    ...
    getName: () => 'reduce',
    getDescription: () => 'Reduces a collection to a single value using a reducer function'
  })
})
...
export const ParallelReduceOperatorService = Effect.gen(function* () {
  ...
  return ReduceOperatorService.of({
    ...
    getDescription: () => 'Reduces a collection in parallel for associative operations'
  })
})
...
export const StreamingReduceOperatorService = Effect.gen(function* () {
  ...
  return ReduceOperatorService.of({
    ...
    getDescription: () => 'Reduces a stream of values with backpressure support'
  })
})