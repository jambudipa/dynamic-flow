 ...
 */
export const FilterOperatorService = Context.GenericTag<FilterOperatorService>('FilterOperatorService')
...
export const DefaultFilterOperatorService = Effect.gen(function* () {
  ...
  return {
    execute: (config: FilterConfig, input: any, context: ExecutionContext): Effect.Effect<FilterResult, OperatorError, never> =>
      (Effect.gen(function* () {
    ...
    validate: (config: FilterConfig) =>
      Effect.gen(function* () {
        ...
        if (config.predicate === undefined && !config.filter && !config.condition) {
          return yield* Effect.fail(new ValidationError({
            field: 'predicate',
...
export const ParallelFilterOperatorService = Effect.gen(function* () {
  ...
  return {
    ...
    validate: (config: FilterConfig): Effect.Effect<boolean, ValidationError, never> =>
...
export const CompositeFilterOperatorService = Effect.gen(function* () {
  ...
  return {
    ...
    getDescription: () => 'Filters with complex predicate compositions (AND, OR, NOT)'
  }
})
