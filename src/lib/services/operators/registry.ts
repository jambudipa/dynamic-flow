 ...
 */
export class OperatorRegistryService extends Effect.Service<OperatorRegistryService>()('OperatorRegistryService', {
  effect: Effect.gen(function* () {
    // Internal registry state - singleton through service
    const operators = yield* Ref.make<HashMap.HashMap<string, UnifiedOperator<any>>>(HashMap.empty())
 ...
 * Test implementation
 */
export const OperatorRegistryTest = Layer.succeed(
  OperatorRegistryService,
  {
    _tag: 'OperatorRegistryService' as const,
    register: () => Effect.void,
    get: (type: string) => Effect.fail(new ValidationError({ message: `Operator ${type} not found`, field: 'type' })),
