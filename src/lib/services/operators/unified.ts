 ...
 */
export class OperatorService extends Effect.Service<OperatorService>()('OperatorService', {
    ...
    const service = {
      ...
      execute: (type: string, config: any, input: any) =>
        Effect.gen(function* () {
          ...
          switch(type) {
            case 'reduce': return yield* reduceService.execute(config, input, context)
            case 'map': return yield* mapService.execute(config, input, context)
            case 'filter': return yield* filterService.execute(config, input, context)
            case 'switch': return yield* switchService.execute(config, input, context)
            case 'loop': return yield* loopService.execute(config, input, context)
            case 'conditional': return yield* conditionalService.execute(config, input, context)
            default: return yield* Effect.fail(new OperatorError({ operator: type, message: 'Unknown operator type' }))
          }
        }),
       ...
       */
      executeWithStrategy: (type: string, strategy: string, config: any, input: any) =>
        Effect.gen(function* () {
       ...
       */
      executeBatch: (operations: Array<{ type: string; config: any; input: any }>) =>
        Effect.all(
          operations.map(op => service.execute(op.type, op.config, op.input)),
       ...
       */
      executePipeline: (operations: Array<{ type: string; config: any }>, initialInput: any) =>
        Effect.reduce(
          operations,
          initialInput,
          (input, op) => service.execute(op.type, op.config, input)
        )
    }
...
export const OperatorServiceLive = Layer.mergeAll(
  Layer.effect(ReduceOperatorService, DefaultReduceOperatorService.pipe(
    Effect.provideService(ExecutionContextService, ExecutionContextService.of({} as any))
  )),
  Layer.effect(MapOperatorService, SequentialMapOperatorService.pipe(
    Effect.provideService(ExecutionContextService, ExecutionContextService.of({} as any))
  )),
  Layer.effect(FilterOperatorService, DefaultFilterOperatorService.pipe(
    Effect.provideService(ExecutionContextService, ExecutionContextService.of({} as any))
  )),
  SwitchOperatorService.Default,
  LoopOperatorService.Default,
  Layer.effect(ConditionalOperatorService, DefaultConditionalOperatorService.pipe(
    Effect.provideService(ExecutionContextService, ExecutionContextService.of({} as any))
  ))
).pipe(
...
export const OperatorServiceParallel = Layer.mergeAll(
  Layer.effect(ReduceOperatorService, ParallelReduceOperatorService.pipe(
    Effect.provideService(ExecutionContextService, ExecutionContextService.of({} as any))
  )),
  Layer.effect(MapOperatorService, ParallelMapOperatorService.pipe(
    Effect.provideService(ExecutionContextService, ExecutionContextService.of({} as any))
  )),
  Layer.effect(FilterOperatorService, ParallelFilterOperatorService.pipe(
    Effect.provideService(ExecutionContextService, ExecutionContextService.of({} as any))
  )),
  SwitchOperatorService.Default,
  LoopOperatorService.Default,
...
export const OperatorServiceTest = Layer.effect(
  OperatorService,
  Effect.gen(function* () {
    const operators = yield* Ref.make<HashMap.HashMap<string, any>>(HashMap.empty())
    ...
    return {
      ...
      listOperatorTypes: () => Effect.succeed(['reduce', 'map', 'filter', 'switch', 'loop', 'conditional']),
