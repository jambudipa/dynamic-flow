 ...
 */
export class LoopOperatorService extends Effect.Service<LoopOperatorService>()('LoopOperatorService', {
  effect: Effect.gen(function* () {
    ...
    return {
       ...
       * Execute loop
       */
      execute: (config: LoopConfig, input: any, context: ExecutionContext): Effect.Effect<LoopResult, OperatorError, never> =>
        Effect.gen(function* () {
          ...
          if (!isWhileLoop && !isTimesLoop) {
            return yield* Effect.fail(new OperatorError({
              operator: 'loop',
              message: 'Loop must have either "while" condition or "times" count',
              cause: { config }
            }))
          }
          ...
          if (isWhileLoop) {
            while (iterations < maxIterations) {
              ...
              const shouldContinue = yield* Effect.try(() => {
                ...
                if (typeof config.while === 'string' && config.while.startsWith('$')) {
              ...
              try {
                // Set loop variables
                yield* context.setVariable(`${config.loop}_index`, iterations)
                yield* context.setVariable(`${config.loop}_iteration`, iterations + 1)
      ...
      validate: (config: LoopConfig) =>
        Effect.gen(function* () {
          ...
          if (hasWhile && hasTimes) {
            return yield* Effect.fail(new ValidationError({
              field: 'while/times',
              message: 'Loop cannot have both "while" and "times" specified'
            }))
          }
       ...
       */
      getDescription: () => 'Executes a block repeatedly based on condition or count'
    }
  })
}) {}