import { Effect, Ref, pipe, HashMap, Option } from 'effect';
...
export interface ExecutionContext {
  ...
  readonly tools?: Array<{
    id: string;
    execute: (input: any) => Effect.Effect<any, any>;
  }>;
}
...
export const IRExecutorServiceImpl = Effect.gen(function* () {
  ...
  ): Effect.Effect<unknown, ExecutionError, never> =>
    Effect.gen(function* () {
  ...
  ): Effect.Effect<Record<string, any>, ExecutionError, never> =>
    Effect.gen(function* () {
  ...
  const resolveValue = (
    value: any
  ): Effect.Effect<any, ExecutionError, never> =>
    Effect.gen(function* () {
    ...
    execute: (ir: IRNode | IR, context?: ExecutionContext) =>
      Effect.gen(function* () {
        ...
        if ('graph' in (ir as any)) {
          ...
          const entryNodeOption = HashMap.get(
            irObj.graph.nodes,
            irObj.graph.entryPoint
          );
          const entryNode = Option.getOrUndefined(entryNodeOption);
          if (!entryNode) {
            return yield* Effect.fail(
              new ExecutionError({
          ...
          // Update context with all nodes (convert HashMap to Map for compatibility)
          const nodesMap = new Map<string, IRNode>();
          HashMap.forEach(irObj.graph.nodes, (node, id) => {
            nodesMap.set(id, node);
          });

          if (!context) {
            context = { nodes: nodesMap };
          } else {
            (context as any).nodes = nodesMap;
          }
        } else {
      ...
      Effect.gen(function* () {
        try {
          ...
          if ('graph' in (ir as any)) {
            ...
            if (!entryNode) {
              yield* addLog(
                `Validation failed: entry point not found: ${irObj.graph.entryPoint}`
              );
              return false;
            }
            node = entryNode;
          ...
          switch (node.type) {
            ...
            case 'sequence': {
              ...
              if (!seqNode.steps || !Array.isArray(seqNode.steps)) {
                yield* addLog(
                  `Validation failed: sequence node missing steps array`
                );
                return false;
              }
              break;
            ...
            case 'parallel': {
              ...
              if (!parNode.branches || !Array.isArray(parNode.branches)) {
                yield* addLog(
                  `Validation failed: parallel node missing branches array`
                );
                return false;
              }
              break;
            ...
            case 'conditional': {
              ...
              if (!condNode.condition) {
                yield* addLog(
                  'Validation failed: conditional node missing condition'
                );
                return false;
              }
              if (!condNode.thenBranch || !Array.isArray(condNode.thenBranch)) {
                yield* addLog(
                  'Validation failed: conditional node missing thenBranch'
                );
                return false;
              }
              break;
            ...
            case 'loop': {
              ...
              if (
                (loopNode.loopType === 'for' || loopNode.loopType === 'map') &&
                !loopNode.collection
              ) {
                yield* addLog(
                  'Validation failed: for/map loop missing collection'
                );
                return false;
              }
              if (loopNode.loopType === 'while' && !loopNode.condition) {
                yield* addLog(
                  'Validation failed: while loop missing condition'
                );
                return false;
              }
              break;
