...
export const OpenAILLMService = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const service: LLMService = {
      ...
      generateStructured: <T>(prompt: string, schema: SchemaType<T>, options?: LLMOptions) =>
        Effect.gen(function* () {
          // Generate with JSON mode
          const jsonOptions = { 
            ...options, 
            systemPrompt: 'You must respond with valid JSON matching the provided schema.'
          }
          
          const response = yield* service.generateCompletion(prompt, jsonOptions)
      ...
      generateStructured: <T>(prompt: string, schema: SchemaType<T>, options?: LLMOptions) =>
        Effect.gen(function* () {
          const response = yield* service.generateCompletion(prompt, {
            ...options,
            systemPrompt: 'Respond only with valid JSON matching the requested schema.'
          })
...
export const TestLLMService = Layer.succeed(
  ...
  LLMService.of({
    ...
    generateStructured: (prompt: any, schema: any) =>
      Effect.succeed({} as any),
    
    streamCompletion: () =>
      Stream.fromIterable(['test', ' stream']),
