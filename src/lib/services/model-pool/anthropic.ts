...
import { ModelPoolService, type ModelInstance, type PoolStats } from './service'
import { PoolError } from '../../errors'
import { ConfigService } from '../config/service'

interface AnthropicModelInstance extends ModelInstance {
...
/**
 * Anthropic model pool implementation
 */
export const AnthropicPoolLive = Layer.effect(
  ModelPoolService,
  Effect.gen(function* () {
    ...
    const instances = yield* Ref.make<Map<string, AnthropicModelInstance>>(new Map())
    ...
    yield* Effect.forEach(
      ...
      (index) => Effect.gen(function* () {
        const instanceId = `anthropic-${index}`
        const instance: AnthropicModelInstance = {
          id: instanceId,
          provider: 'anthropic',
          model: 'claude-3-opus-20240229',
          inUse: false,
          lastUsed: new Date(),
          apiKey: config.models.anthropicApiKey || '',
    ...
    return {
      acquire: (model: string) => Effect.gen(function* () {
        ...
        try {
          ...
          const instanceId = yield* pipe(
            ...
            Effect.timeout(Duration.seconds(config.models.timeout / 1000 || 30)),
            Effect.catchAll(() => 
              Effect.fail(new PoolError({ 
          ...
          const updatedInstance: AnthropicModelInstance = {
            ...instance,
      ...
      release: (instance: ModelInstance) => Effect.gen(function* () {
        // Update instance state
        yield* Ref.update(instances, map => {
          const newMap = new Map(map)
      ...
      releaseAll: () => Effect.gen(function* () {
        ...
        yield* Effect.forEach(
          ...
          id => Queue.offer(availableQueue, id)
        )
      }),
      ...
      resize: (size: number) => Effect.gen(function* () {
        ...
        if (size < currentSize) {
          ...
          const instanceIds = Array.from(currentInstances.keys()).slice(-toRemove)
      ...
      health: () => Effect.gen(function* () {
        ...
        if (!config.models.anthropicApiKey) return false
