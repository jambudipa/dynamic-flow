...
export class StreamExecutor {
                  ...
                  node?.id ?? '',
                  {
                    ...
                    result:
                      event.type === 'node-complete'
                        ? (event as { result?: unknown | undefined }).result
                        : undefined,
                    error:
  ...
  private executeToolNodeStream(
    node: FlowNode & { toolId?: string | undefined },
    flow: ValidatedFlow,
    stateRef: Ref.Ref<FlowState>
  ): Stream.Stream<FlowEvent, ExecutionError> {
    if (
    ...
    return Stream.fromEffect(Ref.get(stateRef)).pipe(
      Stream.flatMap((state) => {
        ...
        const emitTokens =
          (node as { config?: { emitTokens?: boolean | undefined } })?.config
            ?.emitTokens !== false;
        ...
        if (isLLM && hasPrompt) {
          ...
          const raw = Stream.unwrap(
            Effect.gen(function* () {
              ...
              return svc.stream((inputs as { prompt: string }).prompt);
            })
          ...
          const run = Stream.fromEffect(eff).pipe(
            ...
            Stream.catchAll((error: { message?: string | undefined }) =>
              Stream.fromIterable<FlowEvent>([
                {
                  type: 'tool-error',
                  timestamp: ts(),
  ...
  private executeParallelNode(
    ...
    const branchEffects = node.branches.map((branch) =>
      Effect.forEach(branch, (nodeId: string) => {
  ...
  private collectResults(state: FlowState): unknown {
    ...
    (state.nodes ?? new Map<string, unknown>()).forEach(
      (nodeState: unknown, nodeId: string | undefined) => {
        if (
          nodeState !== null &&
          nodeState !== undefined &&
