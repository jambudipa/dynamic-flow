...
const logNormal = (stats: Stats) =>
  Effect.sync(() => {
    console.log(`✅ Normal range: avg=${stats.average.toFixed(2)}`);
    return { status: 'normal', stats };
  });

const generateDetailedReport = (stats: WithAlert) =>
  Effect.sync<WithAlert & Report>(() => {
    console.log('\n📊 Generating detailed anomaly report...');
    return {
      ...stats,
      report: {
...
const generateSummaryReport = (stats: WithAlert) =>
  Effect.sync<WithAlert & Report>(() => {
    console.log('\n📄 Generating summary report...');
    return {
      ...stats,
...
const monitoringFlow = (
  ...
  { summary: string; details: WithAlert & Report },
  never,
  never
> =>
  pipe(
    ...
    Flow.doIf((stats) => stats.average > 75, {
      onTrue: (stats: Stats) =>
        pipe(
          sendAlert(`High average detected: ${stats.average.toFixed(2)}`),
          Flow.andThen(() =>
            Effect.succeed<WithAlert>({
              ...stats,
      ...
      onFalse: (stats: Stats) =>
        pipe(
          ...
          Flow.andThen(() =>
            Effect.succeed<WithAlert>({
              ...stats,
              alertStatus: 'not_needed' as const,
            })
          )
        ),
    }),
    ...
    Flow.andThen((stats: WithAlert) =>
      Flow.doIf((s: WithAlert) => s.max - s.min > 50, {
        onTrue: (s: WithAlert) =>
          pipe(
            Effect.succeed(s),
            Flow.tap(() =>
