...
const userInfoFlow = pipe(
  ...
  Flow.andThen((user) => {
    console.log(
      `📝 Fetching department ${user.departmentId} for ${user.name}...`
    );
    return pipe(
  ...
  Flow.andThen(({ user, department }) => {
    console.log(
      `📝 Fetching manager ${department.managerId} for ${department.name}...`
    );
    return pipe(
...
export const runExample = async () => {
  ...
  const collected = await Effect.runPromise(Flow.runCollect(userInfoFlow));
  ...
  console.log('\n— Streaming (events) —');
  await Stream.runForEach(Flow.runStream(userInfoFlow), (event) =>
    Effect.sync(() => {
